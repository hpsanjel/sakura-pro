...
import { X, DollarSign, CreditCard, Calendar, FileText, Upload, Camera, Receipt, AlertCircle, CheckCircle, Plus, Minus, Edit3 } from "lucide-react"
...
export default function EditPaymentModal({ isOpen, onClose, payment, onSuccess }: EditPaymentModalProps) {
  ...
  const feeTotalAmount = payment.fee.amount
  const currentPaidAmount = payment.fee.paidAmount
  const otherPaymentsTotal = currentPaidAmount - payment.amount
  const maxAmount = feeTotalAmount - otherPaymentsTotal
  ...
  useEffect(() => {
    if (isOpen && payment) {
      ...
      setExistingReceipts(payment.receiptUrls || [])
      
      // Populate transfer details if they exist
      if (payment.transferDetails) {
        const details = typeof payment.transferDetails === 'string' 
          ? JSON.parse(payment.transferDetails) 
          : payment.transferDetails
        setTransferDetails({
          bankName: details.bankName || "",
          accountNumber: details.accountNumber || "",
          accountHolder: details.accountHolder || "",
          transferDate: details.transferDate ? new Date(details.transferDate).toISOString().split('T')[0] : "",
  ...
  const removeExistingReceipt = (url: string) => {
    setRemovedReceipts(prev => [...prev, url])
    setExistingReceipts(prev => prev.filter(r => r !== url))
  }
  ...
  const uploadToCloudinary = async (files: File[]): Promise<string[]> => {
    ...
    for (const file of files) {
      ...
      formData.append('feeId', payment.fee.id)
      formData.append('studentName', payment.fee.student.name)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentAmount > maxAmount) {
        setError(`Amount cannot exceed maximum of ¥${maxAmount.toLocaleString()} (considering other payments)`)
        return
      }

      // Upload new receipts to Cloudinary if available
      let newReceiptUrls: string[] = []
      if (uploadedFiles.length > 0) {
        newReceiptUrls = await uploadToCloudinary(uploadedFiles)
      }

      // Combine existing receipts (minus removed ones) with new ones
      const finalReceiptUrls = [...existingReceipts, ...newReceiptUrls]
      ...
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            ...
            <p className="text-gray-600 mt-1">{payment.fee.title}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 bg-gray-50 border-b">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-medium text-gray-900">{payment.fee.student.name}</span>
          </div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">Total Fee Amount</span>
            <span className="text-sm font-medium text-gray-900">{formatCurrency(feeTotalAmount)}</span>
          </div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">Other Payments</span>
            <span className="text-sm font-medium text-green-600">{formatCurrency(otherPaymentsTotal)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-600">Maximum for this Payment</span>
            <span className="text-lg font-bold text-indigo-600">{formatCurrency(maxAmount)}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900">Payment Receipts</h3>
              <button
                type="button"
            ...
            {existingReceipts.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Current Receipts:</h4>
            ...
            {receiptPreview && (
              <div className="mt-4">
                <h4 className="text-sm font-medium text-gray-700 mb-2">New Receipt Preview:</h4>
